...
function connectDB(){
    ...
    try{
        $bdd = new PDO('mysql:host=localhost;dbname=php_final_project;charse=utf8', 'root', '');

        // TODO: Penser quand le site est fait à commenter ou enlever cette ligne de debug
...
function mustBeConnected(){
    ...
    // Si $_SESSION['user'] n'existe pas, alors l'utilisateur est déconnecté
    if(!isConnected()){
        header('HTTP/1.0 403 Forbidden');
...
// Fonction qui permettra d'afficher la classe CSS 'active' sur un lien du menu uniquement si $pageToTest contient le nom de la page actuelle. NE PAS OUBLIER L'ESPACE APRES LE ECHO ENTRE LES ''
function setActiveIfPageIs($pageToTest){

    // On récupère le nom de la page actuelle via la fonction basename ( https://www.php.net/fr/function.basename.php )

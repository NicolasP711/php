<?php
    ...
    if(
        ...
        if(!preg_match('/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[ !"#\$%&\'()*+,\-.\/:;<=>?@[\\\\\]\^_`{\|}~]).{8,4096}$/', $_POST['password'])){
            $errors[] = 'Le mot de passe doit comprendre au moins 8 caractères dont 1 lettre minuscule, 1 lettre majuscule, 1 chiffre et 1 caractère spécial.';
        }
        ...
        if(!isset($errors)){
            ...
            // Vérification que l'email n'est pas déjà utlisé
            ...
            if(!isset($errors)){
                ...
                if($statut){
                    ...
                    $errors[] = 'Il y a eu un problème lors de la création du compte. Veuillez réessayer plus tard';
                }
            }
        }
    }
